...
def cli():
    """
    A Click group that serves as an entry point for a CLI application. This group can be extended with commands or other groups.
    """
    pass
...
@click.command()
@click.option(
    "--mode", "-m", type=click.Choice(["folder", "file"]), default="folder", help="Mode of operation: folder or file."
)
@click.argument("path", type=click.Path(exists=True))
@click.option(
    "--format", is_flag=True, show_default=True, default=False, help="Format the file after adding the docstrings."
)
def run(mode, path, format):
    ...
    if not os.getenv("OPENAI_API_KEY"):
        logging.error("PROMPT is required. have you ran `autodoc configure`?")
        exit(1)
    auto_doc = AIAutodocService(assistant_id=assistant_id, assistant_instructions=assistant_instructions, prompt=prompt)
    if not assistant_id:
...
def configure():
    """
    A command-line interface command that initializes a new configuration.

    This command creates a new instance of the Config class and calls its initialize method to set up the initial
